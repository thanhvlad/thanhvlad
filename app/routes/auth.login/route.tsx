 ...
 * library allows on this route; anything else is sent to the App Store listing,
 * where installs start.
 */
export const loader = async ({ request }: LoaderFunctionArgs) => {
  const url = new URL(request.url);
  if (!url.searchParams.get("shop")) throw redirect(APP_LISTING_URL);
  return { errors: loginErrorMessage(await login(request)) };
};

/** Nothing posts here any more; an old bookmarked form lands on the listing. */
export const action = async () => redirect(APP_LISTING_URL);
        ...
        <Card>
          <BlockStack gap="300">
            ...
            <Text as="p">
              ...
              Apps, or <Link url={APP_LISTING_URL}>install it from the Shopify App Store</Link>.
            </Text>
          </BlockStack>
        </Card>
      </Page>
